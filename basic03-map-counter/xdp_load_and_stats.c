			...
			struct stats_record *stats_prev)
{
	...
	__u64 kbytes;
	double mbs; /* Mbits per sec */ 

	/* Assignment#2: Print other XDP actions stats  */
	__u32 actions[XDP_ACTION_MAX] = {
		XDP_ABORTED,
		XDP_DROP,
		XDP_PASS,
		XDP_TX,
	...
	for (size_t i = 0; i < XDP_ACTION_MAX; i++) {
		char *fmt = "%-12s %'11lld pkts (%'10.0f pps)"
			" %'11lld Kbytes (%'6.0f Mbits/s)"
			" period:%f\n";
		const char *action = action2str(actions[i]);
		...
		mbs   = kbytes * 8 / 1000 / period;

		printf(fmt, action, rec->total.rx_packets, pps, kbytes, mbs, period);
	}
}
